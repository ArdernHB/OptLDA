...
In our case, with unequal sample size, we are effectively doing a combination of the two leave out many procedures (i.e. stratified cross validation): a Monte-Carlo random partitioning because we are randomly subsetting our data to equal sample size; and then a k-fold because ultimately when we run the CCV exercise we are doing this on equally partitioned data. This means although we benefit from the k-fold approach of treating each specimen as a training and test sample, we still may have specimens in out total dataset that never appear in any of our sampling iterations. As a result, it is important to carry out this multiple iterations many times and for this process to be feasible within R the OptLDA functions employ parallel processing.
...
From this k-fold CCV we can see the central tendency is much clearly close to the expected 50% mark and although the range is similarly large as the leave-one-out approach, the variance is much lower.
