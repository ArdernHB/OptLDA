...
LDACVPar <- function(DiscriminationData, GroupMembership, EqualIter=100, SampleSize=NA, Verbose=FALSE, ShapeGPA=FALSE, Sliding=NULL, Bending=TRUE, SlidingLMindex=NULL, SizeShape=FALSE, PClim=10){
  ...
  ParEqualIter <- function(DiscriminationData, GrpMem, ShapeGPA, Sliding, SlidingLMindex, Bending, ParVerbose=Verbose, SizeShape, PClim, SampleSize){
...
LDACVStepwisePar <- function(DiscriminationData, GroupMembership, EqualIter=100, SampleSize=NA, Verbose=FALSE, ShapeGPA=FALSE, Sliding=NULL, SlidingLMindex=NULL, Bending=TRUE, SizeShape=FALSE, PClim=10, PlotResults=TRUE, CombinePlots=FALSE){
  ...
  ParEqualIterStepwise <- function(DiscriminationData, GrpMem, ShapeGPA, Sliding, SlidingLMindex, Bending, ParVerbose=Verbose, SizeShape, PClim, SampleSize){
    ...
    if (ShapeGPA==TRUE){
      BalDataShape <- DiscriminationData[,,BalancingGrps$IndexedLocations]
      invisible(utils::capture.output(BalRes <- Morpho::procSym(BalDataShape, sizeshape = SizeShape, outlines = Sliding, SMvector = SlidingLMindex, bending = Bending)))
  ...
  a <- 1
  ParResults <- foreach::foreach(a = 1:EqualIter, .combine = ParOutput) %dopar% {
    ParEqualIterStepwise(DiscriminationData, GrpMem=GroupMembership, ShapeGPA=ShapeGPA, Sliding=Sliding, SlidingLMindex=SlidingLMindex, Bending=Bending, ParVerbose=Verbose, PClim=PClim, SizeShape = SizeShape, SampleSize = SampleSize)
  }
